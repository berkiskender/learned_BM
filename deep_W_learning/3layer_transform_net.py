...
for epoch in range(0,Nepoch):
    ...
    for step in range(subset_steps): # batch size
        ...
        x_ref1 = W1@(F.relu(W2@(F.relu(W3@x_noisy[:, ref_ind:ref_ind+1]))))
        x_matched = W1@(F.relu(W2@(F.relu(W3@x_noisy[:, match_inds[ref_ind,:]]))))
        x_unmatched = W1@(F.relu(W2@(F.relu(W3@x_noisy[:, unmatch_inds[ref_ind,:]]))))
        ...
        loss_reg = - gamma_0 * W1.slogdet()[1] + gamma_1 * torch.sum(torch.abs(W1)**2) - gamma_0 * W2.slogdet()[1] + gamma_1 * torch.sum(torch.abs(W2)**2)-gamma_0 *W3.slogdet()[1] + gamma_1*torch.sum(torch.abs(W3)**2)
